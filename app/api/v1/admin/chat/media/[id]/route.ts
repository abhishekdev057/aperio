  ...
  try {
    ...
    if (!mediaId) {
      ...
      if (mediaId) await query(`UPDATE chat_messages SET media_id=$2, media_size=$3 WHERE id=$1`, [messageId, mediaId, data.length]);
        ...
        return new NextResponse(new Uint8Array(data), { headers: { "content-type": msg.mediaMime || mime, "cache-control": "private, max-age=3600" } });
      }
    }
    ...
    return new NextResponse(new Uint8Array(media.data), {
      headers: {
        ...
        "content-disposition": `inline; filename="${(media.name || "file").replace(/"/g, "")}"`,
